...
This hot-reload is for the **host-side developer**, not the caged agent: as described under "Config compilation", the cage denies in-cage write of the watched files (`config-ro.sb`), so a prompt-injected agent cannot use the watcher to widen its own egress. (One residual: if the *developer* adds, mid-session, an include pointing at a new in-project file, that new file is not in the launch-time `config-ro.sb` set and is agent-writable until the next run — a human-initiated, narrow gap, since the agent cannot edit the top config to add includes itself.)
