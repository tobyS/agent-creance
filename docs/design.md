...
- Agent reading host files outside `./` — SSH keys, AWS creds, 1Password state, browser cookies are all denied, and writing anywhere else under `$HOME` is denied too. (The deliberate, narrow exceptions: read of the single mitmproxy CA PEM via the `ca.sb` fragment — see "State directory" / AC-0034 — needed so env-var-CA clients trust the proxy, with the CA private key staying unreadable; and, in v0.1, the real `~/.claude` and `~/.claude.json`, mounted/granted read-write so the caged agent uses the host's Claude config and login out of the box — the config-cage deferral, see "The proxy and the credential story" and AC-0046 — plus the scoped Keychain grant from spike S2. The `~/.claude` mount must not widen anything else in `$HOME`.)
...
- Claude's own OAuth token — the agent has to use it, so it's reachable by the agent (on macOS via a Seatbelt-granted ACL to the one login-Keychain item, not a file in the config dir — see "The proxy and the credential story"; in v0.1 the real `~/.claude` is mounted read-write). A prompt injection can therefore still exfiltrate the token, but only through an allowlisted destination, since direct egress is blocked. Note this is *every* allowlisted host that accepts an agent-controlled body (e.g. a `POST` to the GitHub API), not just `api.anthropic.com`: the allowlist *narrows* the exfil surface to your allowed POST targets, it does not eliminate it. A `mode: passthrough` host is the *least* observable target of all — uninspectable by design — so the audit log cannot even record what was sent there; this is the deliberate trade for not intercepting it. (v0.1 does **not** close the config-persistence vector: with `~/.claude` mounted read-write, a prompt-injected agent can plant a hook, MCP server, or skill that fires on your next un-caged Claude run — a deliberate v0.1 scope cut, tracked for revision as AC-0046.)
...
safehouse:
  ...
  # v0.1: agent-creance mounts your real ~/.claude (and ~/.claude.json)
  # read-write into the cage automatically — the config cage is
...
**`include:` paths are confined.** Because a cloned, untrusted project carries its own `.agent-creance.yaml`, an `include:` entry may only point *into the declaring file's own directory subtree* or *into the global config dir (`~/.config`)*. An absolute path, a `~/` expansion, or a `..` that escapes both is rejected with a clear error before the file is read — so a repo's config cannot pull in `~/.ssh/id_rsa`, `~/.aws/credentials`, or `/etc/passwd`, and an out-of-scope target's contents can never leak through a parse-error message. The implicit global baseline is loaded directly (not as an include), so it is always honored regardless of where it lives; includes that the global itself declares are subject to the same rule (and permitted via the `~/.config` grant). The check is lexical on the resolved path; an in-scope symlink whose target lies elsewhere is still followed, so symlink-target confinement is not part of this guarantee (the cage's in-cage config-write deny remains the backstop against an agent rewriting the include graph).
...
- An allow rule for the package's `homepage`, if the registry reports one. If the homepage is a bare host (e.g. `https://react.dev/`), the rule covers the whole host. If it carries a path (e.g. `https://someuser.github.io/coollib/`), the rule is scoped to that path prefix (`github.io/someuser/coollib/`) — this is what stops a package hosted on a shared, path-multiplexed domain (`github.io`, `*.readthedocs.io`, …) from allowlisting every other tenant's content. Subdomain-based shared hosting (`coollib.vercel.app`) is already correctly scoped, since each project gets its own host. The one gap a bare host can't cover is a **shared-apex** host where many tenants live behind one apex keyed only by path (`sourceforge.net/projects/<x>`, `pear.php.net/package/<x>`): there is no path to scope to, and a host-wide allow would cover every co-tenant — so for a small, curated set of known shared-apex hosts (data, like the forge table) a bare-host homepage emits **no** rule at all, and the user can `agent-creance allow ...` the specific path if they need it. A path-carrying homepage on the same apex is still scoped normally.
...
The state directory lives *outside* the source tree on purpose. The agent runs with the project mounted read-write, so anything security-critical kept inside `./` would be editable by the very process it constrains — and because mitmproxy hot-reloads its policy on mtime change, an in-tree `policy.json` could be rewritten by a prompt-injected agent to allow itself anything. Keeping the compiled policy, the enforcer, the lock, the audit log, and the credential broker's socket out-of-tree means the agent cannot rewrite its own controls — and, in the socket's case, cannot reach the very secrets injection exists to keep from it. None of these files are mounted into the cage at all. Only host-side processes touch them — the CLI, mitmproxy, the broker, and safehouse-at-launch.
...
The one thing that stays in the source tree is the human-authored config (`.agent-creance.yaml` and any `include:`d files) — it's version-controlled input. During a run, `agent-creance` watches this config and its include graph and recompiles `policy.json` on a host-side edit, so a developer tweaking egress rules mid-session sees them take effect without a restart (see "In-session config hot-reload"). The live policy the proxy enforces is always the out-of-tree compiled artifact.

Because that watcher would otherwise hand the caged agent a way to widen its own egress — the project is mounted read-write, so a prompt-injected agent could edit `.agent-creance.yaml` and have the watcher recompile it — the cage denies in-cage *write* of every file in the resolved include graph via a final `--append-profile` Seatbelt fragment (`config-ro.sb`, AC-0053). The agent can still read its config; it just cannot edit the input that drives the live policy. The fragment lists the launch-time resolved set, which is sufficient: the agent cannot edit the top config to introduce a new (writable) include either.
...
### In-session config hot-reload

The `allow`/`deny`/`import` commands already recompile `policy.json`, which the enforcer's mtime poll reloads — that is why a command-driven rule change reaches a live cage. AC-0053 extends the same delivery to **hand-edits of the source config**: for the duration of a run, `agent-creance` watches the project config and every file in its resolved include graph (included fragments plus the implicit global baseline). A debounced change recompiles `policy.json`; the enforcer's existing poll applies it to the live proxy within ~1s — no restart.

The watcher is **session-only** — started after the proxy attaches and torn down (goroutine joined, OS watcher closed) when the agent exits. It uses fsnotify behind the `internal/sysdep` `FileWatcher` seam, watching the parent directories of the config files and filtering by name (editors save atomically by rename, which would break a watch placed on the file itself). An **invalid edit changes nothing**: the compiler only overwrites `policy.json` on a successful, atomic compile, so a syntax/validation error leaves the last-good policy in force; the watcher prints a warning and a later valid save recompiles. Reload feedback goes to stderr (run's stdout belongs to the agent), distinct from the agent's foreground output. A watcher that cannot start warns but never blocks the session.

This hot-reload is for the **host-side developer**, not the caged agent: as described under "Config compilation", the cage denies in-cage write of the watched files (`config-ro.sb`), so a prompt-injected agent cannot use the watcher to widen its own egress.

**Known limitation — mid-session includes added by the developer.** `config-ro.sb` lists the include graph *as resolved at launch*; the Seatbelt profile is fixed for the life of the cage and cannot be updated live. So if the **developer** adds, mid-session, an `include:` pointing at a new in-project file, that new file is not in the deny set and stays agent-writable until the next `agent-creance run` — at which point the watcher's recompile picks up the new include and the next launch's `config-ro.sb` covers it. The gap is narrow and human-initiated: the agent cannot reach it on its own, because editing the top config to introduce the include is itself denied. Editing files *already* in the launch-time graph (the common case) is fully protected. Closing the gap entirely would require re-deriving the deny set on each reload and pushing it into the running cage — a live profile-update mechanism the cage does not have today; tracked as a possible follow-up rather than built here.
...
mitmproxy writes a JSONL file at `egress.jsonl` in the project's state directory (`~/.cache/agent-creance/projects/<hash>/`), mode `0600`. Each entry records timestamp, method, URL, decision (allow / soft-deny / hard-deny), matching rule, and response status. No request headers are logged at all — so an `Authorization`, `Cookie`, or `X-Api-Key` header can never reach the log — and the logged URL has its **query string stripped entirely** (only scheme, host, and path are kept). Query strings are where credentials ride (bearer tokens, signed-URL signatures, session ids) under arbitrary parameter names, so dropping the whole query, rather than redacting a denylist of known names, is the safe shape; the host and path keep the entry's debugging value. Keeping the log out-of-tree matters for integrity: the agent runs with `./` writable, so an in-tree audit log could be truncated or doctored by the very process it records. `agent-creance logs` is how you read it — it never needs to live in the repo.
...
**v0.1 deliberately ships without the config cage.** The headline guarantee agent-creance delivers in v0.1 is *network egress filtering*, not config isolation. So v0.1 takes the simple path: it mounts the real `~/.claude` (and `~/.claude.json`) **read-write** into the cage and does **not** redirect `CLAUDE_CONFIG_DIR`. The caged agent therefore uses your normal Claude config and login with zero ceremony — it reads its real `~/.claude.json` account state and the plain `Claude Code-credentials` Keychain item, exactly as un-caged Claude Code does. An ephemeral config dir was actively in the way here: it strips the user's global `CLAUDE.md`/hooks/settings, *and* because setting `CLAUDE_CONFIG_DIR` changes the Keychain service name Claude Code derives (it appends a hash of the config-dir path), it broke credential lookup outright — the first fully-launching caged session failed its login with a misleading "port 0 in use" OAuth error. **The accepted cost** is that the config-persistence vector is *re-opened*: with `~/.claude` writable, a prompt-injected agent can plant a hook, MCP server, or skill that fires on your next un-caged Claude run on the host. v0.1 documents this honestly rather than pretending the cage covers it. Re-introducing executable-config isolation without breaking auth or the out-of-the-box login is tracked as **AC-0046**.

**The credential stays in the one shared store.** On macOS, Claude Code keeps the OAuth token in the login **Keychain** (service `Claude Code-credentials`, account = the login short name), not a config file — typically there is no `~/.claude/.credentials.json` on the machine. agent-creance does **not** clone it per project; every caged session uses the same Keychain item, exactly as un-caged Claude Code does. The generated Seatbelt profile grants the cage exactly two things, per spike [S2](#open-spikes-resolve-before-build) as completed by the AC-0033 battery (2026-07-04): `mach-lookup` to `com.apple.SecurityServer` (the `securityd` broker — sufficient on its own for the modern `SecItem` API Claude Code uses) plus file-level read-write on `~/Library/Keychains/login.keychain-db*` **and** the `~/Library/Keychains/.fl*` AtomicFile lock files (the legacy `SecKeychain` stack — `/usr/bin/security`, keytar — opens, flocks, and atomically rewrites the db client-side; S2's baseline profile allowed all file reads, which masked the read/lock half of this requirement until the battery's `kc-read`/`kc-write` vectors ran against the real cage). This is the corrected, file-level grant — earlier drafts said "the item's ACL," which S2 showed is inaccurate: the legacy path needs file-level access to the keychain db, and that is the minimum that lets in-cage refresh rotate the shared token. Honest trade: raw read of `login.keychain-db` exposes the (encrypted) db bytes to the caged agent — an offline-attack surface on the login password — but raw *write* was already granted, and the credential item itself is readable via `securityd` by design, so the widening is marginal.

**The target design (deferred past v0.1 — AC-0046).** Executable config and the credential ultimately want *opposite* handling: the credential is shared state every session must read *and* write, while `settings.json`/hooks/MCP/`skills/` are executable config that should not survive into a later un-caged run. The intended split points the agent at a throwaway `CLAUDE_CONFIG_DIR` under the state dir (seeded with minimal sanitized account state so login still "just works"), keeps the real `~/.claude` read-only or absent, and closes the config-persistence vector — while still reaching the one shared Keychain item. The hard part AC-0046 must solve is the Keychain service-name derivation noted above: setting `CLAUDE_CONFIG_DIR` makes Claude Code look up a per-dir-hashed item name, so an isolated config dir and the shared credential pull against each other unless brokered carefully.

**File-based credential fallback (out of scope for v0.1).** v0.1 targets the Keychain default. Some setups instead keep a file-based `~/.claude/.credentials.json` (older versions, managed configs, CI-style machines); v0.1 does **not** support that path. `run`/`doctor` detect the situation (Keychain item absent but a credentials file present) and refuse with a clear message — "caged sessions require a Keychain-stored credential; run `claude` login on the host" — instead of a confusing mid-session auth failure. Supporting file-based credentials properly is deferred.
...
**Host-side, in memory, never on disk.** References (`op://`, `keychain://`, `env://`) are resolved on the host through a `SecretResolver` seam **before the sandbox applies**, once per proxy spawn. The resolved value is handed to the **credential broker** over an **inherited file descriptor** — never argv (visible in `ps`), never the environment (readable by same-uid processes via `KERN_PROCARGS2`), never disk (macOS has no `tmpfs`, and an encrypted file is theater unless its key is in the Keychain, at which point you would store the secret there and drop the file). Per-request resolution was rejected: `op read` costs 200–500ms and is rate-limited. Resolution happens **only on the spawn path**, never when an agent attaches to an already-running proxy, because `op://` can prompt for Touch ID and must not re-prompt every attach.

**The credential broker (AC-0069b).** The broker is a small Go daemon — this same binary, re-executed as `agent-creance broker` — spawned as a sibling of `mitmdump` and torn down with it by the same last-out `Detach`. It holds the resolved tokens and serves them to the addon over a **unix socket** (`broker.sock`, mode `0600`, in the out-of-tree state dir), one request per connection. The addon holds no secrets of its own: it fetches the token once **per injected request**, so a credential exists in the Python process only for as long as a request needs it, and a **rotated** token takes effect on the very next request without restarting the proxy — the channel short-lived minted tokens (AC-0069a) need. The wire protocol carries `expires_at`, and the broker refuses to serve an expired token (472) rather than sending a dead one upstream to collect a 401.

Why a *daemon* rather than a goroutine in `run`: the proxy is refcounted per project and deliberately outlives the invocation that started it whenever a second agent is attached. A broker owned by one run session would vanish under a live proxy, and the surviving agent's injected requests would 472 forever.

The socket's mode, and the fact that the cage cannot reach it, are the whole access control. There is no bearer token and no peer-credential check, because a check would prove nothing: the caged agent runs as the **same uid** as `mitmdump`, so `LOCAL_PEERCRED` reports the same uid for the attacker and the legitimate client. What does the work is unreachability — the state dir is never mounted, `(deny network*)` covers a unix-socket connect, and a generated `broker.sb` fragment denies the socket path explicitly — asserted by the `broker-socket` vector in the adversarial battery.

**What the broker's memory hygiene honestly buys.** The broker `mlock`s each token, wipes the buffer it replaces on rotation, zeroes everything on SIGTERM, and disables core dumps. That is worth doing, and it is *hygiene, not a control*. `mlock(2)` works on darwin (only `mlockall` is `ENOSYS`), but macOS swap is encrypted by default, the adversary in this model shares the user's uid, and Go copies stacks and spills registers — so a `[]byte` wipe cannot chase every derived copy, and Go 1.26's `runtime/secret`, which could, is a no-op on darwin. Anything that can read this process's memory has the token. What actually bounds the damage of a leaked credential is its **scope** and its **TTL** (AC-0069a), not memory protection. We say so rather than implying the broker is a vault.

The practical consequence of spawn-only resolution is unchanged: adding an `inject` rule to a *running* cage hot-reloads the rule but not the broker's secrets, so that host answers 472 until the proxy respawns.

**Minted short-lived tokens (AC-0069a).** A static injected token is repo-scoped but *not* time-bounded: a leak is bounded in scope but lives until you rotate it by hand. Phase 2 brings the state-of-the-art model (GitHub Actions' `GITHUB_TOKEN`) host-side — a per-session, scoped credential that **expires** and is auto-refreshed — so what a leak buys an attacker is bounded in *time* as well as scope. Two minted forms exist, both declared as a `credentials:` entry with a sub-block instead of a `source:`:

- **GitHub App installation token.** The host signs an RS256 JWT with the App's private key (`iat` backdated 60s for clock skew, `exp` ≈9min, `iss` = the App's client ID), discovers the installation for the target repo, and exchanges the JWT for a **repo-scoped installation token that lives ≤1h** (non-configurable) with a `permissions` down-scope. The App **private key never enters the cage** — only the minted, short-lived, repo-scoped token is ever injected. One repo per credential (multiple repos = multiple credentials). Register it with `agent-creance credential add-github-app <name> --key keychain://… --client-id … --repo owner/name --perm contents=read …`; the key is a `keychain://`/`op://`/`env://` reference (PKCS#1 PEM), resolved host-side at spawn like any other. Onboarding: register a GitHub App under your account (webhooks can be off — it is a pure token-minting App), install it on the one repo, and store its downloaded PEM key in the keychain.

- **OAuth2 refresh grant.** The host exchanges a stored **refresh token** for a short-lived access token (one form-encoded `grant_type=refresh_token` POST, RFC 6749 §6), computing expiry from `expires_in`. The refresh token never enters the cage; only the access token is injected. The first (and, for now, only) target is **Google Drive with the non-sensitive `drive.file` scope** (`drive.readonly`/`drive` are restricted scopes that trigger Google's app verification). Register it with `agent-creance credential add-oauth2 <name> --refresh-token keychain://… --client-id …` (endpoint and scope default to Google Drive). Obtaining the refresh token is a one-time, host-side, out-of-cage consent: `agent-creance credential authorize <name>` runs the RFC 8252 native-app loopback flow (a listener on `127.0.0.1:<random port>`, PKCE S256, the system browser opens Google's consent screen), exchanges the returned code, and stores the refresh token at the credential's `keychain://` reference. Use a **Desktop app** OAuth client (its "secret" is not confidential). Caveats to know: a consent screen still in **Testing** publishing status expires the refresh token in **7 days**; Google caps you at **100 refresh tokens per account per client**; disuse for 6 months expires it.

**The broker is the refresh-loop owner.** Minting and refresh live in the broker because it is the one long-lived host-side process, and it already custodies the served token. The enriched fd-3 payload delivers a per-credential *spec* (`static` | `github_app` | `oauth2`, carrying the already-resolved key material plus the non-secret parameters) rather than a flat token map; this is a Go→Go contract inside one binary (the broker is a re-exec of the same `agent-creance`), so it needs no cross-version framing. For each minted credential the broker runs a per-credential goroutine: mint the first token, then re-mint before expiry (GitHub margin ≈5min, OAuth2 ≈2min, with jitter) and swap it into the store via the same rotation entry point a static token never uses. The Python enforcer is **unchanged** — it fetches a token by name and renders it through the existing header/template shape, indifferent to whether the token was resolved or minted.

**Refresh-failure semantics: stale-then-472-at-expiry.** If a refresh fails, the broker keeps serving the previous, still-valid token (it does *not* overwrite it) and retries on a backoff so a transient failure self-heals. Only once `expires_at` passes without a successful refresh does the broker answer **472** — the human-recoverable refusal (re-authorize / unlock the store), never a dead token sent upstream to collect a 401. This reuses the existing 472 body verbatim; the 470/471/472 taxonomy is untouched. On last-agent teardown the broker **best-effort-revokes** minted GitHub tokens (`DELETE /installation/token`) before wiping — a failure is swallowed, since the token expires within the hour regardless. `doctor` surfaces a non-prompting authorization check for a project's minted credentials (a `keychain://` item is checked for existence without reading it; an unauthorized OAuth2 credential is an advisory warning with the `credential authorize` next step, never a non-zero exit).

**The honest bound is unchanged, and now sharpened.** Minting bounds the *leak window* — a stolen minted token dies in ≤1h and is repo-scoped — but it does **not** change the `/graphql` public-read posture below: token lifetime bounds the window, not public-read *scope*. Scoped REST stays the default; `/graphql` stays a documented opt-in. The app key / refresh token never entering the cage, plus TTL + scope, are what actually bound a leak — not the broker's memory hygiene.
...
**Per-project scoping falls out of the architecture.** The proxy is a host-side daemon refcounted per project (keyed by state-dir hash), so four concurrent cages on four repos are four proxies with four `policy.json`s — and four brokers on four sockets. Each resolves and holds only its own project's credential. Four repos means four independently scoped tokens with no shared state — which is precisely what neutralizes the shared keychain.
...
**Trade-offs accepted.** Custody now sits in the Go broker rather than in Python memory (AC-0069b) — but see above for what that honestly buys: `mlock` and zeroization are hygiene, not a control, and anything that can read the broker's memory has the token. Static tokens are repo-scoped but **not time-bounded**; minted short-lived tokens (GitHub App installation tokens, OAuth2 refresh — AC-0069a) close that gap, bounding a leak in *time* as well as scope, and the broker is the rotation channel they write into (see "Minted short-lived tokens" above). And injection removes *non-Claude* secrets from the agent's reach, not Claude's own — see the section above for why that one is irreducible.
...
- **v0.2:** credential injection — Phase 1 (static `op://`/`keychain://`/`env://` references, overwrite + fail-closed, 472, Bearer gold-plated with GitHub as the validated flagship) is done, as is the host-side unix-socket secret broker (AC-0069b), which moved custody into Go and gave the channel rotation, and Phase 2's minted short-lived tokens (GitHub App installation tokens + OAuth2 refresh — AC-0069a), which the broker refreshes before expiry and `credential authorize` bootstraps. Also: DNS resolver in the proxy with blocklists (Hagezi, OISD), structured deny-decision log alongside the main audit log, additional ecosystem generators (`pyproject_toml`, `cargo_toml`, etc.) as contributions arrive.
