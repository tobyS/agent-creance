...
- **Credential injection does not scope *public* reads.** An injected repo-scoped GitHub token bounds writes and private reads only; opening `api.github.com/graphql` (one endpoint whose target repo is in the uninspected body) lets the caged agent read *any* public repo — the ingestion vector the cage exists to close. Default to scoped REST (`/repos/{owner}/{repo}/…`); treat `/graphql` as a documented opt-in risk. See `docs/design.md` "Credential injection".
- **Never commit a personal secret reference (`op://…`, `keychain://…`) to this public repo's `.agent-creance.yaml`.** A committed `source:` that only resolves on your machine makes every other contributor's cage answer 472 on that host. Keep credential setup in the docs as an example with a placeholder; this project uses tmt (files) for tickets, so it needs no GitHub injection in its own config.
