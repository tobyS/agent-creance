---
plan: thoughts/shared/plans/2026-06-18-AC-0055-init-git-allowlist.md
ticket: AC-0055
updated: 2026-06-18
---

# AC-0055 implementation status

- [ ] Phase 1 — Extend forge table with api.github.com; export RepositoryRules/NormalizeRepoURL
- [ ] Phase 2 — internal/gitremote detection package (.git/config parser)
- [ ] Phase 3 — Build git-remote config.Rule allow/deny sets
- [ ] Phase 4 — Wire into runInit (flag, prompt, render, report)
- [ ] Phase 5 — Tests: testscript + goldens
- [ ] Phase 6 — Docs (design.md) + ticket close

## Notes
- Started 2026-06-18.
