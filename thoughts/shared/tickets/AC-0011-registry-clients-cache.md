...
- [ ] npm client fetches from `registry.npmjs.org`; Packagist client from `packagist.org`; both extract `homepage` + `repository`.
- [ ] Metadata is cached per package; a cache entry younger than 30 days is reused without a network call; older triggers a refresh.
- [ ] HTTP access is injectable (fake transport in tests); no unit test performs a real network call.
- [ ] Live calls exist only under `//go:build integration`.
...
- [ ] Exact npm + Packagist JSON shapes for `homepage`/`repository` (and version-range resolution to "latest matching")?
- [ ] Cache-poisoning concern: do we validate the cache file's integrity on read?
